...
ANSWER: This loop is already a constant time complexity so there is no better solution
...
// ANSWER:
O(1) no because constant is the fastest/easiest complexity. 
...
function squareEveryDigit(number) {
  return Number(('' + number).split('').map(function (val) { return val * val;}).join(''));
}
